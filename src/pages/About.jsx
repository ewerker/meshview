      ...
      <div className="flex-1 max-w-2xl mx-auto w-full p-6 space-y-8">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {features.map(f => (
              <div key={f.title || f.titleKey} className="bg-white dark:bg-slate-800 rounded-lg p-4 border dark:border-slate-700 flex gap-3 items-start">
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${f.color}`}>
                  <f.icon className="w-5 h-5" />
                </div>
                <div>
                  <div className="font-semibold text-sm text-slate-700 dark:text-slate-200">{f.titleKey ? t(f.titleKey) : f.title}</div>
                  <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 leading-relaxed">{f.descKey ? t(f.descKey) : f.desc}</div>
                </div>
              </div>
            ))}
          </div>
